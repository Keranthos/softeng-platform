...
func initGinMode() {
	if m := strings.TrimSpace(os.Getenv("GIN_MODE")); m == gin.ReleaseMode || m == gin.TestMode {
		gin.SetMode(m)
		return
	}
...
func main() {
	...
	ragAgentSvc := service.NewRagAgentService(db, cfg)
	ragAgentHandler := handler.NewRagAgentHandler(ragAgentSvc)
	...
	r.Use(func(c *gin.Context) {
		...
		if p != "/health" && p != "/live" {
			log.Printf("[DEBUG] Request: %s %s", c.Request.Method, p)
		}
		c.Next()
	})
	...
	// 认证路由（全局限流：每 IP 每分钟最多 45 次，防爆破/刷注册）
	auth := r.Group("/auth")
	auth.Use(middleware.AuthRateLimit(45, time.Minute))
	{
	...
	agent.Use(middleware.AuthMiddleware(), middleware.AdminMiddleware())
	{
		agent.POST("/rag", ragAgentHandler.Chat) // RAG：与前端一致，仅管理员（需 Bearer + role）
	}

	r.GET("/api/ws/stream", handler.WsDemoStream) // WebSocket 演示：查询参数 token + 管理员（见 handler）
