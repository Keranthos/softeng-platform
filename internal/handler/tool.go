...
import (
	...
	"log"
	"net/http"
	"softeng-platform/internal/pkg/safety"
...
func (h *ToolHandler) SubmitTool(c *gin.Context) {
	...
	if msg := safety.ValidateToolSubmit(req.Name, req.Link, req.Description, req.DescriptionDetail); msg != "" {
...
func (h *ToolHandler) AddComment(c *gin.Context) {
	...
	if msg := safety.ValidateComment(req.Content); msg != "" {
		response.Error(c, http.StatusBadRequest, msg)
		return
	}
